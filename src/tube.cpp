...
void Tube::display()
{
    ...
    for (int i = 0; i < vertices.size(); i++)
    {
        ...
        double px1, py1;
        transfo2D(vertices[i], vertices[(i + 1) % vertices.size()], 0, 0, px1, py1, DEPTH_TUBE_COEF, false);

        double px2, py2;
        transfo2D(vertices[i], vertices[(i + 1) % vertices.size()], normAB, 0, px2, py2, DEPTH_TUBE_COEF, false);
